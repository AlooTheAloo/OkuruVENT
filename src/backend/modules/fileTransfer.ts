 ...
 */
export function createModuleForClient(
  socket: Socket,
  mainwindow: BrowserWindow,
) {
  // Create handler for this client
  ipcMain.handle(
    "Application:FileTransfer:RequestFileTransfer:" + socket.id,
    (evt, hostname: string) => {
      dialog.showOpenDialog(mainwindow, {}).then(res => {
        if (res.canceled) {
          ...
          rpcInvoke(
            ...
            currentSenderTransfers,
            currentAwaitingSenderTransfers,
          );
        }
      });
    },
  ...
  socket.on(
    "ACK:Transfer:FileRequestTransfer",
    (fileTransferID, accepted, hostname) => {
      let targetTransfer = currentAwaitingSenderTransfers.find(
        x => x.id == fileTransferID,
      );
      if (targetTransfer == null) {
        console.log(
          "Your transfer somehow got accepted by someone you didn't ask to transfer to. Interesting.",
        );
        return;
      }
      targetTransfer.hostname = hostname;
      currentAwaitingSenderTransfers = currentAwaitingSenderTransfers.filter(
        x => x.id != fileTransferID,
      );
      if (accepted) {
  ...
  socket.on(
    "ACK:Transfer:FileTransferPacket",
    (
      fileTransferID,
      unixMSTimeStamp,
      packetID,
      ...
      if (positionBytes * 1e6 < fileSize) {
        ...
        const transfer: Transfer = currentSenderTransfers.filter(
          x => x.id == fileTransferID,
        )[0]; // get transfer
        currentSenderTransfers = currentSenderTransfers.filter(
          x => x.id != fileTransferID,
        ); // Remove transfer
...
function SendPacket(
  ...
  if (targetTransfer == null || targetTransfer == undefined) {
    console.log(
      "Someone requested a file you haven't shared with them. Interesting.",
    );
    return;
  }
  ...
  fs.open(targetTransfer.filepath, "r", (status, fd) => {
    // Open target file
    if (status) {
      // Error
    ...
    if (positionBytes * 1e6 + targetMB * 1e6 > fileSize) {
      // Last packet too large
 ...
 */
function calculateNextPacketSize(
  lastSize: number,
  lastTime: number,
