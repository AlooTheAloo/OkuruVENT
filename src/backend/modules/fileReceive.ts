 ...
 * @returns {void}
 */
export function createModuleForServer(
  socket: Socket,
  mainwindow: BrowserWindow,
): void {
  eventHandler.on(
    `Application:DisconnectSocket:${socket.id}`,
    function handler() {
      socket.disconnect();
      socket.offAny();
  ...
  socket.on(
    ...
    (
      ...
      console.log(
        "created listener for Application:RespondToTransfer:" + transferID,
      );
      ipcMain.handle(
        "Application:RespondToTransfer:" + transferID,
        (evt: Event, response: boolean) => {
          console.log("received response " + response);
          if (!response) {
            awaitingTransfers = awaitingTransfers.filter(
              x => x.id != transferID,
            );
            socket.emit(
              ...
              .then(res => {
                if (!res.canceled) {
                  ...
                  transfers.push({
                    ...
                    socketID: socket.id,
                    filename:
                      res.filePath.split(sep)[
                        res.filePath.split(sep).length - 1
                      ],
                    fileSize: fileSize,
                    progress: 0,
                    hostname: hostname,
      ...
      shownNotification.on("click", () => {
        dialog
          ...
          .then(res => {
            if (res.canceled) {
              socket.emit(
                "AK:Transfer:FileRequestTransfer",
                transferID,
                false,
                getHostName(),
              );
              ...
              transfers.push({
                ...
                socketID: socket.id,
                filename:
                  res.filePath.split(sep)[res.filePath.split(sep).length - 1],
                fileSize: fileSize,
                progress: 0,
                hostname: hostname,
              });
  ...
  socket.on("Transfer:FileTransferComplete", fileTransferID => {
    ...
    shownNotification.on("click", () => {
      // Clicked on noti, let's open filepath
