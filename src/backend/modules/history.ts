...
export function addToHistoryFile(newTransfer:Delivery){
    const fileContents:Delivery[] =JSON.parse(readFileSync(`${appDataPath}User${ sep }history.txt`).toString());
    fileContents.push(newTransfer);
    writeFileSync(`${appDataPath}User${ sep }history.txt`, JSON.stringify(fileContents));
}
...
export function fetchHistory():Delivery[]{
   return  JSON.parse(readFileSync(`${appDataPath}User${ sep }history.txt`).toString());
}